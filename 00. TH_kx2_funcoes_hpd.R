...
###############################################################################
########### Função de calculo de extremos (intervalo) da posteriori ###########
###############################################################################

posterior_minmax <- function(posterior_pars # lista com k pares de valores da priori (para calcular posteriori)
                             ,d # números de desvio padrão pra mais e menos 
                             ){
  medias_beta <- mapply(function(pars) {
    pars[1]/(pars[1] + pars[2]) ## média da beta = a/(a+b)
    }, posterior_pars)
  sd_beta <- mapply(function(pars) {
    sqrt((pars[1]*pars[2])/((pars[1] + pars[2])^2*(pars[1] + pars[2]))) ## var da beta = ab/(a+b)^2(a+b+1)
  }, posterior_pars)
  min_beta <- medias_beta - d*sd_beta
  max_beta <- medias_beta + d*sd_beta
  return(list(min_beta = min_beta, max_beta = max_beta))
}


###############################################################################
########################### Função de calculo de HPD ##########################
###############################################################################
